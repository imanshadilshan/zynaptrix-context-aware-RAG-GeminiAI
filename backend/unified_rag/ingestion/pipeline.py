...
async def process_manual_async(file_path: str, manual_id: str):
    """
    Ingestion Pipeline: Structural Parsing -> Concurrent Enrichment -> Parallel Embedding.
    """
    print(f"🏗️ [Pipeline] Starting high-precision ingestion for {manual_id}...")
    ...
    # 3. Concurrent Enrichment (Gemini-based)
    print(f"🧠 [Pipeline] Stage 3/4: Concurrent Gemini Enrichment (Captions + Tables)...")
    ...
    sem = asyncio.Semaphore(5) # Limit concurrent Gemini API calls
    ...
    print(f"✨ [Pipeline] SUCCESS: {manual_id} fully ingested.")
